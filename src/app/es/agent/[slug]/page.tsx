...
export async function generateMetadata({
  ...
  const description = `Publicaciones de ${agent.displayName} en terrenoSV.`;
...
export default async function AgentPageEs({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const agent = await fetchAgentBySlug(slug).catch(() => null);
  if (!agent) notFound();
  ...
  return <AgentView agent={agent} listings={listings} locale="es" />;
}
