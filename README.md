...
- **Static export** (`output: "export"` in `next.config.ts`), which builds to
  plain HTML/CSS/JS in `/out`, deployable directly to IONOS shared hosting via
  FTP. No server required. To later upgrade to full server rendering
  (real-time SSR, ISR, API routes), remove `output: "export"` and deploy to
