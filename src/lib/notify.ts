...
export async function subscribeToLaunch(email: string, honeypot: string): Promise<NotifyResult> {
  try {
    const res = await fetch(NOTIFY_ENDPOINT, {
      method: "POST",
      headers: { "Content-Type": "text/plain" },
      body: JSON.stringify({ email, hp: honeypot, secret: NOTIFY_SHARED_SECRET }),
    });
