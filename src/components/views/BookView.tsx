...
export default function BookView({ locale }: { locale: Locale }) {
  ...
  return (
    <div className="mx-auto flex max-w-4xl flex-col items-center gap-10 px-4 py-16 text-center sm:px-6 sm:py-24">
      <div className="relative h-80 w-56 overflow-hidden rounded-2xl shadow-2xl">
        <Image src="/book-cover.png" alt="Moving to El Salvador" fill unoptimized className="object-cover" priority />
      </div>

      <div>
        <p className="text-sm font-semibold uppercase tracking-wide text-accent-warm">{dict.bookPage.eyebrow}</p>
        <h1 className="mt-2 text-3xl font-bold text-foreground sm:text-4xl">{dict.bookPage.title}</h1>
