...
export async function generateMetadata({
  ...
  const description = listing.description.es || listing.description.en;
  return {
    title: listing.title,
    description,
...
export default async function ListingPageEs({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const listings = await fetchListings().catch(() => []);
  const listing = listings.find((l) => l.slug === slug);
  if (!listing) notFound();

  return <ListingDetailView listing={listing} locale="es" />;
}
